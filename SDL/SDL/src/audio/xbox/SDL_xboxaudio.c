...
#include "xb_PCM.h"
#define XA2_CPU_MASK XboxThread4|XboxThread5
...
static SDL_AudioDevice *Audio_CreateDevice(int devindex)
{
	...
	result = XAudio2Create(&sound, 0, XAUDIO2_DEFAULT_PROCESSOR); //XAUDIO2_DEFAULT_PROCESSOR ); //XA2_CPU_MASK ); //XboxThread5 );
	if ( result != S_OK ) {
		return(0);
	}
...
static void XboxDX_ThreadInit(_THIS)
{
	XSetThreadProcessor(GetCurrentThread(), 4 ); //XA2_CPU_MASK); //XboxThread1;
	//SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_ABOVE_NORMAL); //THREAD_PRIORITY_NORMAL
}
	...
	IXAudio2SourceVoice *sndbuf, WAVEFORMATEX *wavefmt, Uint32 chunksize)
{
	...
	IXAudio2_CreateSourceVoice(sndObj, &sndbuf, wavefmt, 0x0000 , 
								XAUDIO2_DEFAULT_FREQ_RATIO, NULL, NULL, NULL ); //XAUDIO2_VOICE_MUSIC; XAUDIO2_VOICE_USEFILTER
...
static int XboxDX_OpenAudio(_THIS, SDL_AudioSpec *spec)
{
	...
	switch ( spec->format & 0xFF ) {
		case 8:
			...
			silence = 0x80;
			waveformat.Format.wBitsPerSample = 8;
			break;
		case 16:
	...
	waveformat.Format.nChannels = spec->channels;
	waveformat.Format.nSamplesPerSec = spec->freq;
	...
	if(waveformat.Format.nChannels == 1){
		waveformat.dwChannelMask = SPEAKER_MONO;
	}
	else if(waveformat.Format.nChannels == 2){
		waveformat.dwChannelMask = SPEAKER_STEREO;
	}
	else if(waveformat.Format.nChannels == 6){
		waveformat.dwChannelMask = SPEAKER_5POINT1;
	}
	else {
		SDL_SetError("Unexpected number of channels: %d\n",waveformat.Format.nChannels);
		waveformat.dwChannelMask = SPEAKER_STEREO;
	}
	...
	waveformat = getPCMFilter(waveformat);
	IXAudio2_CreateSourceVoice(sound, &mixbuf, (WAVEFORMATEX*)&waveformat, 0x0000 , 
								XAUDIO2_DEFAULT_FREQ_RATIO, NULL, NULL, NULL ); //XAUDIO2_VOICE_MUSIC;XAUDIO2_VOICE_FILTER
