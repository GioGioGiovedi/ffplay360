...
static void keyboard_update(void)
{
	...
	if(LastSubKey != 0) {
		SDL_PrivateKeyboard(SDL_RELEASED, TranslateKey(LastSubKey, &keysym, 1));
		LastSubKey = 0;
}
	...
	if(FunctionKeyDown) {
		// Let's adjust for the function key on the chatpad
		switch(m_Key.VirtualKey) {
			case 0xBC:
				LastSubKey = ':';
				break;
			case 0x31:
			...
			case 0x30:
				...
				break;
			case 0x08:
				LastSubKey = VK_ESCAPE;
				break;

		}

		SDL_PrivateKeyboard(SDL_PRESSED,  TranslateKey(LastSubKey, &keysym, 0));	
...
#if(MOUSE)		// can have a real mouse attached
