...
struct private_yuvhwdata {
	LPDIRECT3DTEXTURE9 pFrameY;
	LPDIRECT3DTEXTURE9 pFrameU;
	LPDIRECT3DTEXTURE9 pFrameV;
...
const char* g_strPixelShaderProgramYUV = 
...
static void XBOX_Present(_THIS);
...
static SDL_VideoDevice *XBOX_CreateDevice(int devindex)
{
	...
	device->CreateYUVOverlay = XBOX_CreateYUVOverlay; /*NULL;*/
...
{	 
	...
	#define CUSTOMFLAG D3DCREATE_BUFFER_2_FRAMES|D3DCREATE_CREATE_THREAD_ON_0
	D3D_PP.BackBufferWidth = VideoMode.dwDisplayWidth;
	D3D_PP.BackBufferHeight = VideoMode.dwDisplayHeight;
	D3D_PP.BackBufferFormat = D3DFMT_X8R8G8B8;
	D3D_PP.PresentationInterval   = D3DPRESENT_INTERVAL_IMMEDIATE;
	if (!D3D_Device)
		IDirect3D9_CreateDevice(D3D,0,D3DDEVTYPE_HAL,NULL,CUSTOMFLAG,&D3D_PP,&D3D_Device); /* D3DCREATE_NO_SHADER_PATCHING decreases performance? */
	else
				...
				int width, int height, int bpp, Uint32 flags)
{
	...
	switch(bpp)
	{
		...
		case 24:
			pitch = width*4;
			bpp = 32;
			pixel_mode = D3DFMT_LIN_A8R8G8B8;
		case 32:
			pitch = width*4;
			pixel_mode = D3DFMT_LIN_A8R8G8B8;
			Rmask = 0x00FF0000;
	...
	if (!have_direct3dtexture && !have_vertexbuffer) {
		...
		ret = IDirect3DDevice9_CreateTexture(D3D_Device,width,height,1, 0,pixel_mode, D3DUSAGE_CPU_CACHED_MEMORY, (D3DTexture**)&this->hidden->SDL_Buttons, NULL);
	   ...
	   D3DXCompileShader( g_strPixelShaderProgram,
		...
		IDirect3DDevice9_CreateVertexBuffer(D3D_Device, sizeof(triangleStripVertices), D3DUSAGE_WRITEONLY,0L, D3DPOOL_MANAGED, &vertexBuffer, NULL );
		...
		D3DDevice_SetSamplerState(D3D_Device, 0, D3DSAMP_MAGFILTER, D3DTEXF_POINT); /*D3DTEXF_LINEAR);*/
		D3DDevice_SetSamplerState(D3D_Device, 0, D3DSAMP_MINFILTER, D3DTEXF_POINT); /*D3DTEXF_LINEAR);*/
	} else {
		
	IDirect3DDevice9_SetTexture(D3D_Device, 0, (D3DBaseTexture *)NULL);
	...
	if ( flags & SDL_RESIZABLE)
	{
		...
		drawRect.top = 0;
		drawRect.left = 0;
		drawRect.right = D3D_PP.BackBufferWidth;
		drawRect.bottom = D3D_PP.BackBufferHeight;		
	} else {		
		drawRect.top = ((D3D_PP.BackBufferHeight/2) - (height /2));
		drawRect.right = ((D3D_PP.BackBufferWidth/2) + (width /2));
		drawRect.left = ((D3D_PP.BackBufferWidth/2) - (width /2));
		drawRect.bottom = ((D3D_PP.BackBufferHeight/2) + (height /2));
	}
...
static LPDIRECT3DTEXTURE9 CreateYUVSurface(_THIS, int width, int height, Uint32 format)
{
	...
	IDirect3DDevice9_CreateTexture(D3D_Device,width,height,1, 0,D3DFMT_LIN_YUY2, D3DUSAGE_CPU_CACHED_MEMORY, (D3DTexture**)&surface, NULL);
...
SDL_Overlay *XBOX_CreateYUVOverlay(_THIS, int width, int height, Uint32 format, SDL_Surface *display)
{
	...
	IDirect3DDevice9_CreateTexture(D3D_Device,width,height,1, 0,D3DFMT_LIN_L8, D3DUSAGE_CPU_CACHED_MEMORY, (D3DTexture**)&hwdata->pFrameY, NULL);
	IDirect3DDevice9_CreateTexture(D3D_Device,width/2,height/2,1, 0,D3DFMT_LIN_L8, D3DUSAGE_CPU_CACHED_MEMORY, (D3DTexture**)&hwdata->pFrameU, NULL);
	IDirect3DDevice9_CreateTexture(D3D_Device,width/2,height/2,1, 0,D3DFMT_LIN_L8, D3DUSAGE_CPU_CACHED_MEMORY, (D3DTexture**)&hwdata->pFrameV, NULL);


	D3DXCompileShader( g_strPixelShaderProgramYUV,
							( UINT )strlen( g_strPixelShaderProgramYUV ),
							NULL,
							NULL,
							"main",
							"ps_2_0",
	...
	// Create pixel shader.
	OverlayShader = D3DDevice_CreatePixelShader( ( DWORD* )pPixelShaderCode->lpVtbl->GetBufferPointer(pPixelShaderCode));
	...
	if( NULL == OverlayDecl )
	{
		static const D3DVERTEXELEMENT9 decl[] =
		{
		 ...
		 IDirect3DDevice9_CreateVertexDeclaration(D3D_Device,decl,&OverlayDecl);
            
	}
 
		// Create vertex shader
	if( NULL == OverlayVShader )
	{
		ID3DXBuffer* pShaderCode;
		if( FAILED( D3DXCompileShader( g_strGradientShader, strlen( g_strGradientShader ),
									   NULL, NULL, "GradientVertexShaderYUV", "vs_2_0", 0,
									   &pShaderCode, NULL, NULL ) ) )
			return 0;
		 
		IDirect3DDevice9_CreateVertexShader(D3D_Device,( DWORD* )pShaderCode->lpVtbl->GetBufferPointer(pShaderCode), &OverlayVShader);
	}
...
int XBOX_DisplayYUVOverlay(_THIS, SDL_Overlay *overlay, SDL_Rect *src, SDL_Rect *dst)
{
	...
	IDirect3DDevice9_SetTexture(D3D_Device, 0, (D3DBaseTexture*)overlay->hwdata->pFrameY);
	IDirect3DDevice9_SetTexture(D3D_Device, 1, (D3DBaseTexture*)overlay->hwdata->pFrameU);
	IDirect3DDevice9_SetTexture(D3D_Device, 2, (D3DBaseTexture*)overlay->hwdata->pFrameV);
	
	D3DDevice_SetVertexDeclaration(D3D_Device, OverlayDecl);
	IDirect3DDevice9_SetStreamSource(D3D_Device, 0,overlayVBuffer,0,sizeof(VERTEX));
	D3DDevice_SetVertexShader(D3D_Device, OverlayVShader );
	D3DDevice_SetPixelShader(D3D_Device, OverlayShader);

	D3DDevice_SetSamplerState(D3D_Device, 0, D3DSAMP_MINFILTER, D3DTEXF_POINT );
	D3DDevice_SetSamplerState(D3D_Device, 0, D3DSAMP_MAGFILTER, D3DTEXF_POINT );
	D3DDevice_SetSamplerState(D3D_Device, 0, D3DSAMP_MIPFILTER, D3DTEXF_POINT );

	D3DDevice_SetSamplerState(D3D_Device, 1, D3DSAMP_MINFILTER, D3DTEXF_POINT );
	D3DDevice_SetSamplerState(D3D_Device, 1, D3DSAMP_MAGFILTER, D3DTEXF_POINT );
	D3DDevice_SetSamplerState(D3D_Device, 1, D3DSAMP_MIPFILTER, D3DTEXF_POINT );

	D3DDevice_SetSamplerState(D3D_Device, 2, D3DSAMP_MINFILTER, D3DTEXF_POINT );
	D3DDevice_SetSamplerState(D3D_Device, 2, D3DSAMP_MAGFILTER, D3DTEXF_POINT );
	D3DDevice_SetSamplerState(D3D_Device, 2, D3DSAMP_MIPFILTER, D3DTEXF_POINT );

	IDirect3DDevice9_DrawPrimitive(D3D_Device,  D3DPT_RECTLIST, 0, 1 );
	//IDirect3DDevice9_Present(D3D_Device,NULL,NULL,NULL,NULL);
...
{	
 	D3DLOCKED_RECT rectY; 
	D3DLOCKED_RECT rectU;
	D3DLOCKED_RECT rectV;
 			
	IDirect3DTexture9_LockRect(overlay->hwdata->pFrameY, 0, &rectY, NULL, 0);
	IDirect3DTexture9_LockRect(overlay->hwdata->pFrameU, 0, &rectU, NULL, 0);
	IDirect3DTexture9_LockRect(overlay->hwdata->pFrameV, 0, &rectV, NULL, 0);

	/* Find the pitch and offset values for the overlay */
	overlay->pitches[0] = (Uint16)rectY.Pitch;
	overlay->pitches[2] = (Uint16)rectU.Pitch;
	overlay->pitches[1] = (Uint16)rectV.Pitch;

	overlay->pixels[0]  = (Uint8 *)rectY.pBits;
	overlay->pixels[1]  = (Uint8 *)rectU.pBits;
	overlay->pixels[2]  = (Uint8 *)rectV.pBits;
...
void XBOX_UnlockYUVOverlay(_THIS, SDL_Overlay *overlay)
{
	...
	IDirect3DTexture9_UnlockRect(overlay->hwdata->pFrameY, 0);
	IDirect3DTexture9_UnlockRect(overlay->hwdata->pFrameU, 0);
	IDirect3DTexture9_UnlockRect(overlay->hwdata->pFrameV, 0);
...
void XBOX_FreeYUVOverlay(_THIS, SDL_Overlay *overlay)
{
	...
	if ( hwdata ) {
		if ( hwdata->pFrameY ) {
			IDirect3DTexture9_Release(hwdata->pFrameY);
		}

		if ( hwdata->pFrameU ) {
			IDirect3DTexture9_Release(hwdata->pFrameU);
		}

		if ( hwdata->pFrameV ) {
			IDirect3DTexture9_Release(hwdata->pFrameV);
		}
		free(hwdata);
		overlay->hwdata = NULL;
	}
