...
#define VEC_ALIGNER(src) ((UNALIGNED_PTR(src)) \
    ? __lvsl( src, 0 ) \
    : __vaddshs(__lvsl( src, 8 ), __vspltish(8)))
                                  ...
                                  const SDL_PixelFormat *dstfmt)
{
    ...
    * We have to assume that the bits that aren't used by other
	...
	const __vector4 plus = {0x00000000,
                            0x04040404,
                            0x08080808,
							0x0C0C0C0C
	                       };
...
static void Blit_RGB888_RGB565Altivec(SDL_BlitInfo *info) {
	...
	__vector4 vgmerge = {
        ...
        0x00120016,
		0x001a001e};
    __vector4 v1 = __vspltish(1);
    __vector4 v3 = __vspltish(3);
	__vector4 v3f = {
        0x003f003f003f003f,
		0x003f003f003f003f
	};
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        ...
        while (width) {
            ...
            vsrc1 = __vperm(vsrc, valpha, vpermute);
            src += 16;
            vsrc = voverflow;
            voverflow = __lvx( src, 15 );
            vsrc = __vperm(vsrc, voverflow, valigner);
            vsrc2 = __vperm(vsrc, valpha, vpermute);
            /* 1555 */
            vpixel = __vpkpx(vsrc1, vsrc2);
            vgpixel = __vperm(vsrc1, vsrc2, vgmerge);
            vgpixel = __vand(vgpixel, vfc);
            vgpixel = __vsl(vgpixel, v3);
            vrpixel = __vsl(vpixel, v1);
            vrpixel = __vand(vrpixel, vf800);
            vbpixel = __vand(vpixel, v3f);
            vdst = __vor(vrpixel, vgpixel);
            /* 565 */
            vdst = __vor(vdst, vbpixel);
            __stvx(vdst, dst, 0 );
...
static void Blit_RGB565_32Altivec(SDL_BlitInfo *info) {
    ...
    __vector4 vpermute;
    __vector4 vf800;
    __vector4 v8 = __vspltisw(8);
    __vector4 v16 = __vadduhs(v8, v8);
    __vector4 v2 = __vspltish(2);
    __vector4 v3 = __vspltish(3);
    /* 
    ...
    */
    ...
    __vector4 vredalpha2 =  (
        __vadduhs(vredalpha1, __vsl(v8, v16))
    );
    /*
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        ...
        while (width) {
            ...
            vR = __vand(vsrc, vf800);
            vB = __vsl(vsrc, v3);
            vG = __vsl(vB, v2);

            vdst1 = __vperm(vR, valpha, vredalpha1);
            vdst1 = __vperm(vdst1, vB, vblue1);
            vdst1 = __vperm(vdst1, vG, vgreen1);
            vdst1 = __vperm(vdst1, valpha, vpermute);
            __stvx(vdst1, dst, 0 );

            vdst2 = __vperm(vR, valpha, vredalpha2);
            vdst2 = __vperm(vdst2, vB, vblue2);
            vdst2 = __vperm(vdst2, vG, vgreen2);
            vdst2 = __vperm(vdst2, valpha, vpermute);
            __stvx(vdst2, dst, 16);
...
static void ConvertAltivec32to32_noprefetch(SDL_BlitInfo *info)
{
    ...
    __vector4 vpermute = calc_swizzle32(srcfmt, dstfmt);
    ...
    while (height--) {
       ...
       while (width) {
            ...
            src += 4;
            width -= 4;
            vbits = __vperm(vbits, voverflow, valigner);  /* src is ready. */
            vbits = __vperm(vbits, vzero, vpermute);  /* swizzle it. */
            __stvx(vbits, dst, 0 );  /* store it back out. */
            dst += 4;
            vbits = voverflow;
        }
