...
/* the permuation vector that takes the high bytes out of all the appropriate shorts 
    (vector unsigned char)(
        ...
        0x0C, 0x1C, 0x0E, 0x1E );
*/
#define VEC_MERGE_PERMUTE() (__vadduhs(__lvsl( (int*)NULL, 0 ), __vspltish(0x0F)))
#define VEC_U32_24() (__vadduws(__vspltisw(12), __vspltisw(12)))
#define VEC_ALPHA_MASK() (__vsl(__vspltish(-1), VEC_U32_24()))
#define VEC_ALIGNER(src) ((UNALIGNED_PTR(src)) \
    ? __lvsl( src, 0 ) \
    : __vadduhs(__lvsl( src, 8 ), __vspltish(8)))

   
#define VEC_MULTIPLY_ALPHA(vs, vd, valpha, mergePermute, v1_16, v8_16) do { \
    /* vtemp1 contains source AAGGAAGGAAGGAAGG */ \
    __vector4 vtemp1 = __vmulfp(vs, valpha); \
    /* vtemp2 contains source RRBBRRBBRRBBRRBB */ \
   __vector4 vtemp2 = __vmulfp(vs, valpha); \
    /* valpha2 is 255-alpha */ \
    __vector4 valpha2 = __vnor(valpha, valpha); \
    /* vtemp3 contains dest AAGGAAGGAAGGAAGG */ \
    __vector4 vtemp3 = __vmulfp(vd, valpha2); \
    /* vtemp4 contains dest RRBBRRBBRRBBRRBB */ \
    __vector4 vtemp4 = __vmulfp(vd, valpha2); \
    /* add source and dest */ \
    vtemp1 = __vadduhs(vtemp1, vtemp3); \
    vtemp2 = __vadduhs(vtemp2, vtemp4); \
    /* vtemp1 = (vtemp1 + 1) + ((vtemp1 + 1) >> 8) */ \
    vtemp1 = __vadduhs(vtemp1, v1_16); \
    vtemp3 = __vsr(vtemp1, v8_16); \
    vtemp1 = __vadduhs(vtemp1, vtemp3); \
    /* vtemp2 = (vtemp2 + 1) + ((vtemp2 + 1) >> 8) */ \
    vtemp2 = __vadduhs(vtemp2, v1_16); \
    vtemp4 = __vsr(vtemp2, v8_16); \
    vtemp2 = __vadduhs(vtemp2, vtemp4); \
    /* (>>8) and get ARGBARGBARGBARGB */ \
    vd = __vperm(vtemp1, vtemp2, mergePermute); \
} while (0)
...
static __vector4 calc_swizzle32(const SDL_PixelFormat *srcfmt,
                                  const SDL_PixelFormat *dstfmt)
{
    /*
     * We have to assume that the bits that aren't used by other
...
#undef RESHIFT  
    ((unsigned int *)(char*)&srcvec)[0] = (rmask | gmask | bmask | amask);
    vswiz = __vadduhs(plus, __vsplth(srcvec, 0));
    return(vswiz);
}
...
static void Blit32to565PixelAlphaAltivec(SDL_BlitInfo *info)
{
    ...
    __vector4 v0 = __vspltish(0);
    __vector4 v8_16 = __vspltish(8);
    __vector4 v1_16 = __vspltish(1);
    __vector4 v2_16 = __vspltish(2);
    __vector4 v3_16 = __vspltish(3);
    __vector4 v8_32 = __vspltish(8);
    __vector4 v16_32 = __vadduws(v8_32, v8_32);
	__vector4 v3f = {
                       0x003f003f, 
					   0x003f003f,
		               0x003f003f, 
    ...
    /* 
        ...
        0x01 - 0x0f odds are zero
    */
	__vector4 vredalpha1 = {
                              0x10000101,
                              0x10020101,
                              0x10040101,
                              0x10060101
	                         };
    __vector4 vredalpha2 = ( __vadduhs(vredalpha1, __vsl(v8_32, v16_32))
    );
    /*
        0x00 - 0x0f is ARxx ARxx ARxx ARxx
        0x11 - 0x0f odds are blue
    ...
    */
    ...
    __vector4 vgreen2 = (  __vadduhs(vgreen1, __vsl(v8_32, v8_32))
    );
	__vector4 vgmerge = {
                         0x00020006,
                         0x000a000e,
    ...
    __vector4 mergePermute = VEC_MERGE_PERMUTE();
    __vector4 vpermute = calc_swizzle32(srcfmt, NULL);
    __vector4 valphaPermute = __vand(__lvsl( (int *)NULL, 0 ), __vspltish(0xC));

    __vector4 vf800 = __vspltish(-7);
    vf800 = __vsl(vf800, __vspltish(8));
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        ...
        while (width) {
            __vector4 valpha;
            __vector4 vsrc1, vsrc2;
            __vector4 vdst1, vdst2;
            __vector4 vR, vG, vB;
            __vector4 vpixel, vrpixel, vgpixel, vbpixel;

            /* Load 8 pixels from src as ARGB */
            voverflow = __lvx( src, 15 );
            vsrc  = __vperm(vsrc, voverflow, valigner);
            vsrc1 = __vperm(vsrc, vsrc, vpermute);
            src += 16;
            vsrc = __lvx( src, 15 );
            voverflow = __vperm(voverflow, vsrc, valigner);
            vsrc2 = __vperm(voverflow, voverflow, vpermute);
            src += 16;

            /* Load 8 pixels from dst as XRGB */
            voverflow = __lvx( dst, 0 );
            vR = __vand(voverflow, vf800);
            vB = __vsl(voverflow, v3_16);
            vG = __vsl(vB, v2_16);
            vdst1 = __vperm(vR, vR, vredalpha1);
            vdst1 = __vperm(vdst1, vB, vblue1);
            vdst1 = __vperm(vdst1, vG, vgreen1);
            vdst2 = __vperm(vR, vR, vredalpha2);
            vdst2 = __vperm(vdst2, vB, vblue2);
            vdst2 = __vperm(vdst2, vG, vgreen2);

            /* Alpha blend 8 pixels as ARGB */
            valpha = __vperm(vsrc1, v0, valphaPermute);
            VEC_MULTIPLY_ALPHA(vsrc1, vdst1, valpha, mergePermute, v1_16, v8_16);
            valpha = __vperm(vsrc2, v0, valphaPermute);
            VEC_MULTIPLY_ALPHA(vsrc2, vdst2, valpha, mergePermute, v1_16, v8_16);

            /* Convert 8 pixels to 565 */
            vpixel = __vpkpx(vdst1, vdst2);
            vgpixel = __vperm(vdst1, vdst2, vgmerge);
            vgpixel = __vand(vgpixel, vfc);
            vgpixel = __vsl(vgpixel, v3_16);
            vrpixel = __vsl(vpixel, v1_16);
            vrpixel = __vand(vrpixel, vf800);
            vbpixel = __vand(vpixel, v3f);
            vdst1 = __vor(vrpixel, vgpixel);
            vdst1 = __vor(vdst1, vbpixel);
...
static void Blit32to32SurfaceAlphaKeyAltivec(SDL_BlitInfo *info)
{
    ...
    __vector4 mergePermute;
    __vector4 vsrcPermute;
    __vector4 vdstPermute;
    __vector4 vsdstPermute;
    __vector4 valpha;
    __vector4 valphamask;
    __vector4 vbits;
    __vector4 v0;
    __vector4 v1;
    __vector4 v8;
    ...
    valpha = __vsplth(valpha, 0);
    vbits = __vspltish(-1);

    ckey &= rgbmask;
    ((unsigned int *)(char*)&vckey)[0] = ckey;
    vckey = __vsplth(vckey, 0);
    ((unsigned int *)(char*)&vrgbmask)[0] = rgbmask;
    vrgbmask = __vsplth(vrgbmask, 0);
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        ...
        if (width > 0) {
            ...
            __vector4 valigner = VEC_ALIGNER(srcp);
            __vector4 vs = __lvx( srcp, 0 );
            width -= extrawidth;
            while (width) {
                __vector4 vsel;
                __vector4 voverflow;
                ...
                /* s = *srcp */
                voverflow = __lvx( srcp, 15 );
                vs = __vperm(vs, voverflow, valigner);
                
                /* vsel is set for items that match the key */
                vsel = __vand(vs, vrgbmask);
                vsel = __vcmpequb(vsel, vckey);

                /* permute to source format */
                vs = __vperm(vs, valpha, vsrcPermute);

                /* d = *dstp */
                vd = __lvx( dstp, 0 );
                vd_orig = vd = __vperm(vd, v0, vsdstPermute);

                VEC_MULTIPLY_ALPHA(vs, vd, valpha, mergePermute, v1, v8);

                /* set the alpha channel to full on */
                vd = __vor(vd, valphamask);

                /* mask out color key */
                vd = __vsel(vd, vd_orig, vsel);
                
                /* permute to dest format */
                vd = __vperm(vd, vbits, vdstPermute);
...
static void Blit32to32PixelAlphaAltivec(SDL_BlitInfo *info)
{
    ...
    SDL_PixelFormat *dstfmt = info->dst;
    __vector4 mergePermute;
    __vector4 valphaPermute;
    __vector4 vsrcPermute;
    __vector4 vdstPermute;
    __vector4 vsdstPermute;
    __vector4 valphamask;
    __vector4 vpixelmask;
    __vector4 v0;
    __vector4 v1;
    __vector4 v8;
    ...
    mergePermute = VEC_MERGE_PERMUTE();
    valphamask = VEC_ALPHA_MASK();
    valphaPermute = __vand(__lvsl( (int *)NULL, 0 ), __vspltish(0xC));
    vpixelmask = __vnor(valphamask, v0);
    vsrcPermute = calc_swizzle32(srcfmt, NULL);
    vdstPermute = calc_swizzle32(NULL, dstfmt);
    vsdstPermute = calc_swizzle32(dstfmt, NULL);
...
#define ONE_PIXEL_BLEND(condition, widthvar) while ((condition)) { \
        ...
        if (width > 0) {
            ...
            while (width) {
                ...
                voverflow = __lvx( srcp, 15 );
                vs = __vperm(vs, voverflow, valigner);
                vs = __vperm(vs, v0, vsrcPermute);

                valpha = __vperm(vs, v0, valphaPermute);
                
                /* d = *dstp */
                vd = __lvx( dstp, 0 );
                vd = __vperm(vd, v0, vsdstPermute);
                vdstalpha = __vand(vd, valphamask);

                VEC_MULTIPLY_ALPHA(vs, vd, valpha, mergePermute, v1, v8);

                /* set the alpha to the dest alpha */
                vd = __vand(vd, vpixelmask);
                vd = __vor(vd, vdstalpha);
                vd = __vperm(vd, v0, vdstPermute);
