...
// used to track loaded modules via PsLoadedModuleList in kernel
...
} LDR_DATA_TABLE_ENTRY, *PLDR_DATA_TABLE_ENTRY; // 100 bytes
...
typedef struct _DEBUG_MONITOR_PTRS{
	DWORD unk1;// 0x0
	DWORD unk2;// 0x4
	DWORD unk3;// 0x8
	PCONTEXT dbgContx;// 0xC
	DWORD flags;// 0x10
	DWORD unk4;// 0x14 - points to stack pointer array?
}DEBUG_MONITOR_PTRS, *PDEBUG_MONITOR_PTRS;
...
typedef struct _KTHREAD{
	...
	LIST_ENTRY IrpList;
	PDEBUG_MONITOR_PTRS DebugMonitorData;
...
//C_ASSERT(sizeof(LDR_DATA_TABLE_ENTRY) == 100);
