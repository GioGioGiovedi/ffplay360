...
extern "C" {
#endif
	VOID ObDereferenceObject(HANDLE Object); // kernel export 261
	BYTE KfAcquireSpinLock(PDWORD spinVar); // kernel export 177
	HRESULT ObLookupAnyThreadByThreadId(DWORD dwThreadId, PHANDLE htObj); // kernel export 266
	VOID KfReleaseSpinLock(PDWORD spinVar, BYTE oldIrql); // kernel export 180
#ifdef __cplusplus
}
#endif
#ifdef _XBOX
/*
HANDLE GetThreadContext(HANDLE hThread, LPCONTEXT lpContext)
{
  return (HANDLE)DmGetThreadContext((DWORD)hThread, (PCONTEXT)lpContext);
}

HANDLE SetThreadContext(HANDLE hThread, LPCONTEXT lpContext)
{
  return (HANDLE)DmSetThreadContext((DWORD)hThread, (PCONTEXT)lpContext);
}
*/
#endif
...
ptw32_RegisterCancelation (PAPCFUNC unused1, HANDLE threadH, DWORD unused2)
{
  ...
  return 0;*/
	XboxThreadRegisterCancelation((DWORD)threadH);
	return 0;
}
