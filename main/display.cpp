...
HRESULT InitD3D()
{
	...
	// Create the Direct3D device. //D3DCREATE_HARDWARE_VERTEXPROCESSING
	if( FAILED( pD3D->CreateDevice( 0, D3DDEVTYPE_HAL, NULL,
		D3DCREATE_BUFFER_2_FRAMES|D3DCREATE_CREATE_THREAD_ON_2|D3DCREATE_CREATE_THREAD_ON_3,
		&d3dpp, &g_pd3dDevice ) ) )
		return E_FAIL;
...
HRESULT InitYuvSurface(
	...
	float fRatioW = width/1280.f;//1.5f
	float fRatioH = height/720.f;//1.5f

	//fAspect = (fRatioW/fRatioH)/(16.f/9.f);
	//fAspect = 0.4f;
